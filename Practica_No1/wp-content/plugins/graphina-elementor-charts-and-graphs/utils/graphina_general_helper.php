...
function graphina_ajax_reload($callAjax,$new_settings,$type,$mainId){
    ?><script>
        let setting = '<?php echo empty($new_settings) ?>' ? graphina_localize.graphinaAllGraphsOptions['<?php echo $mainId ?>'].setting_date : <?php echo json_encode($new_settings); ?>;

        if(typeof getDataForChartsAjax !== "undefined" && '<?php echo $callAjax; ?>' === "1") {
            if( !['mixed'].includes('<?php echo $type; ?>' )){
                getDataForChartsAjax(setting , '<?php echo $type; ?>', '<?php echo $mainId; ?>');
            }
            if(setting['iq_'+'<?php echo $type ?>'+ '_can_chart_reload_ajax'] && setting['iq_'+'<?php echo $type ?>'+ '_can_chart_reload_ajax'] === 'yes' ){
                let ajaxIntervalTime = parseInt(setting['iq_'+'<?php echo $type ?>'+ '_interval_data_refresh']) * 1000;
